/*
    wchar.c - A mostly working PCLink server for IDEDOS 0.9x
...
static int compcp2(const void *a, const void *b) {
    return (*(unsigned int *)a & ~0xff) - (*(unsigned int *)b & ~0xff);
}
...
static void set_cp(void) {
    ...
    qsort(revcp, sizeof revcp / sizeof *revcp, sizeof *revcp, compcp);
}
...
size_t wcrtomb(char *s, wchar_t wc, mbstate_t *ps) {
    ...
    if (cp == NULL) set_cp();
    c = wc << 8;
    ch = (unsigned int *)bsearch(&c, revcp, sizeof revcp / sizeof *revcp, sizeof *revcp, compcp2);
    if (ch != NULL) {
        *s = *ch;
        return 1;
    }
